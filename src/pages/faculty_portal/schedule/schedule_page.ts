...
function countScheduleBlocks(schedule: InstructorSchedule): number {
  return schedule.slots.reduce((sum, slot) => sum + Object.values(slot.values).filter(Boolean).length, 0)
}

export function renderfaculty_schedule_page(): string {
  const currentInstructor = [...INSTRUCTOR_SCHEDULES].sort((a, b) => countScheduleBlocks(b) - countScheduleBlocks(a))[0]
